...
collar, and **Camera**, live video from the house.

Python: FastAPI, Jinja templates, plain CSS and plain JavaScript. No build
step, no JavaScript framework, no npm. It runs on a machine at home because
...
| `uv run kona probe --out probe-out\round7` | Ask Fi's API what it knows, redacted, into `summary.md`. The only way to learn what the collar exposes; Fi has no public API. |
